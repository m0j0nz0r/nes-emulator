...
class CustomLogger implements Logger {
  constructor() {
    this._targetLog = fs
      .readFileSync(path.join(__dirname, 'testRoms/nestest.log'), 'utf-8')
      .split('\n');
  }
...
function cpuTestLog(emulator: Emulator) {
  ...
  log +=
    ' P:' + emulator.cpu.status.toString(16).toUpperCase().padStart(2, '0');
  log +=
    ' SP:' +
    emulator.cpu.stackPointer.toString(16).toUpperCase().padStart(2, '0');
  log += ` CYC:${emulator.cpu.cycle}`;
...
export function cpuTest(this: any) {
  ...
  const paletteData = fs.readFileSync(
    path.join(__dirname, 'palettes/2C02G_wiki.pal')
  );
  ...
  emulator.cpu.on('fetch', cpuTestLog.bind(null, emulator)),
