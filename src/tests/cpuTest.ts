...
function compareCpuState(state: CpuState, expectedState: CpuState): string | null {
  if (state.busAddr !== expectedState.busAddr) {
    return `ADDR expected: ${expectedState.busAddr.toString(16)} | got: ${state.busAddr.toString(16)}`;
  }
  if (state.busData !== expectedState.busData) {
    return `DATA expected: ${expectedState.busData.toString(16)} | got: ${state.busData.toString(16)}`;
  }
  ...
  if (state.cpuA !== expectedState.cpuA) {
    return `A expected: ${expectedState.cpuA.toString(16)} | got: ${state.cpuA.toString(16)}`;
  }
  if (state.cpuX !== expectedState.cpuX) {
    return `X expected: ${expectedState.cpuX.toString(16)} | got: ${state.cpuX.toString(16)}`;
  }
  if (state.cpuY !== expectedState.cpuY) {
    return `Y expected: ${expectedState.cpuY.toString(16)} | got: ${state.cpuY.toString(16)}`;
  }
  if (state.cpuStatus !== expectedState.cpuStatus) {
    return `P expected: ${expectedState.cpuStatus.toString(16)} | got: ${state.cpuStatus.toString(16)}`;
  }
  if (state.cpuStackPointer !== expectedState.cpuStackPointer) {
    return `SP expected: ${expectedState.cpuStackPointer.toString(16)} | got: ${state.cpuStackPointer.toString(16)}`;
  }
...
function comparePpuState(state: PpuState, expectedState: PpuState): string | null {
...
function compareEmulationState(state: EmulationState, expectedState: EmulationState): string | null {
...
export async function cpuTest(): Promise<boolean> {
  return new Promise<boolean>((resolve) => {
    ...
    try {
      ...
      const rom = fs.readFileSync(path.join('src/tests/testRoms/nestest/nestest.nes'));
      const targetLog = fs
        .readFileSync(path.join('src/tests/testRoms/nestest/nestest.log'), 'utf-8')
        .split('\n');
      let logLineIndex = 0;
      ...
      emulator.cpu.on('fetch', () => {
        ...
        const comparisonResult = compareCpuState(state.cpuState, expectedState.cpuState);
        if (comparisonResult) {
          console.error(`Mismatch at line ${logLineIndex}: ${comparisonResult}`);
          resolve(false);
          emulator.stop();
          return;
        }
